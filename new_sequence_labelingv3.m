...
output_folder = './output_sequence_labeling';
...
for file_idx = 1:length(dataset_files)
    ...
    % Clear constraint field for events without a fixation type
    for i = 1:length(EEG.event)
        if strcmp(EEG.event(i).fix_type, '[]') %&& ...
           %(strcmp(EEG.event(i).type, '1311') || strcmp(EEG.event(i).type, '1312'))
            EEG.event(i).constraint = '[]';
        end
    end
    
        % change EEG.event.type from 1311 and 1312 to just fixation
    for i = 1:length(EEG.event)
        EEG.event(i).type = 'fixation';
    end

    %% delete rows that don't have both constraint and fixation type
    % the function below may be the greatest thing to happen to Matlab
